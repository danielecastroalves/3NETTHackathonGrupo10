...
namespace HealthMed.WebApi.Controllers
{
    ...
    [ApiController]
    [Route("v1")]
    public sealed class AppointmentSchedulingController : CommonController
    {
        /// <summary>
        /// Constructor
        /// </summary>
        /// <param name="mediator">Mediator DI</param>
        public AppointmentSchedulingController(IMediator mediator) : base(mediator) { }
        ...
        [SwaggerResponse((int)HttpStatusCode.BadRequest, "Bad Request - Invalid input or missing required parameters")]
        [SwaggerResponse((int)HttpStatusCode.Unauthorized, "Unauthorized - Invalid credentials or authentication token")]
        public async Task<IActionResult> GetAppointmentSchedulingAsync(int crmNumber, DateTime date, CancellationToken cancellationToken)
        {
